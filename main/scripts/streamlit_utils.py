...
def add_custom_header_and_footer(header_and_footer_color, logo_image_path, header_background_path, background_image, title, subtitle, more_info_url):
    ...
    <div class="custom-footer">
        Confidential and Proprietary. Copyright © 2025
    </div>
    '''
...
def setup_header_and_footer():
    ...
    more_info_url = "https://github.com/KailashLohar"
...
def handle_sidebar_inputs():
    """Handle user inputs from the sidebar."""
    st.write("")
    st.write("")
    st.write("")
    ...
    uploaded_files = st.file_uploader(
        ...
        help="Upload PDB, PDBQT, and Config File."
    )

    pdb_file, pdbqt_file, config_file, statuses = process_uploaded_files(uploaded_files)
...
def process_uploaded_files(uploaded_files):
    ...
    pdb_file, pdbqt_file, config_file = None, None, None
...
def display_upload_summary(statuses):
    ...
    """.format(
        pdb_status="PDB file uploaded ✅" if statuses["pdb_file"] else "PDB file not uploaded ❌",
        pdbqt_status="PDBQT file uploaded ✅" if statuses["pdbqt_file"] else "PDBQT file not uploaded ❌",
        config_status="Config file uploaded ✅" if statuses["config_file"] else "Config file not uploaded ❌",
    )
...
def run_plip(selected_folder, plc_all_ligands):
    ...
    for pdb_file in pdb_files:
        ...
        try:
            ...
            st.write(f"Standard Error (stderr): {e.stderr.decode()}")
            raise
...
def generate_plip_tables_html(filtered_df):
    ...
    filtered_df = filtered_df.rename(columns=lambda col: col.replace('num_', '#') if col.startswith('num_') else col)
    colgroup_html = ''.join(
...
def plot_heatmap(df):
    ...
    df = df.rename(columns=lambda col: col.replace('num_', '#') if col.startswith('num_') else col)
    ...
    plt.title("Summary of ligand-protein interactions", fontsize=16, 
              fontname='Times New Roman', color='#006064', fontweight='bold', pad=20)
    plt.tight_layout()
...
def collect_pdb_files(output_plip_path):
    ligand_files = [
        os.path.join(root, file)
        for root, _, files in os.walk(output_plip_path)
        for file in files if file.startswith('plipfixed.')
    ]
    if not ligand_files:
        st.error("No SDF files found in output_plip_files.")
    return ligand_files
...
def visualize_3d_structures(output_plip_path, receptor_pdb_path, ligand_file_path):
    try:
        ...
        filtered_columns = [col for col in plip_results.columns if col.startswith('rsnr_') or col.startswith('restype_') or col == 'Name' or col == 'all_rsnr']
        df_all_residues = plip_results[filtered_columns]
        
        def create_residues(row):
            residues = []
            for col in df_all_residues.columns:
                if col.startswith('rsnr_'):
                    interaction_type = col.replace('rsnr_', '')
                    restype_col = f'restype_{interaction_type}'
                    if restype_col in df_all_residues.columns:
                        rsnr_list = eval(row[col])
                        restype_list = eval(row[restype_col])
                        residues.extend([f"{restype}{rsnr}" for restype, rsnr in zip(restype_list, rsnr_list)])
            return list(dict.fromkeys(residues))
        
        df_all_residues = df_all_residues.copy()
        df_all_residues['residues'] = df_all_residues.apply(create_residues, axis=1)
        df_all_residues = df_all_residues[['Name', 'all_rsnr', 'residues']]
        ...
        with col1:
            ...
            viewer.setStyle({'model': 0}, {'cartoon': {'color': '#FFFFFF'}})

            for res, color in residues_color_map.items():
                # st.write(f"res: {res}, color: {color}")
            ...
            viewer.setStyle({'model': 1}, {'stick': {}, 'sphere': {'scale': 0.25}})            
            viewer.addSurface(py3Dmol.VDW, {
                'opacity': 0.4,
                'colorscheme': {
                    'gradient': [
                        {'value': 0.0, 'color': '#FFD700'},  # Gold
                        {'value': 0.5, 'color': '#FFECB3'},  # Light gold
                        {'value': 1.0, 'color': '#FFA500'},  # Orange tint for reflection
                    ]
                }
            })
        ...
        with col2:
            ...
            for res, color in residues_color_map.items():
                residue_name = None
                for residues_list in df_all_residues['residues']:
                    residue_name = next((name for name in residues_list if str(res) == ''.join(filter(str.isdigit, name))), None)
                    if residue_name:
                        break
    
                if residue_name:
                    label_html += f"<li style='margin-top:0px; margin-bottom:5px;'><span style='display:inline-block; width:15px; height:15px; background-color:{color}; margin-right:10px;'></span>{residue_name}</li>"
...
def extract_trimmed_name(file_path):
    match = re.search(r'output_plip_files[\/_](\w+)', file_path)
    if match:
...
def visualize_data(output_plip_path, receptor_pdb_path, ligand_folder_path, selected_ligand_file):
    ...
    if receptor_pdb_path and st.button("Show Visualization", key="visualization_button"):
        with st.spinner("Preparing visualization..."):
            ...
            pdb_files = collect_pdb_files(output_plip_path)
            selected_pdb_file_path = None
            for pdb_file in pdb_files:
                if selected_ligand_file.replace('_out.sdf', '') in pdb_file:
                    selected_pdb_file_path = pdb_file
                    break
            
            if selected_pdb_file_path:
                st.session_state.selected_pdb_file_path = selected_pdb_file_path
                
                trimmed_name = extract_trimmed_name(selected_pdb_file_path)
                
                with open(selected_pdb_file_path, 'rb') as pdb_file:
                    pdb_data = pdb_file.read()
                    b64 = base64.b64encode(pdb_data).decode()
                
                st.markdown(
                    f"<div style='text-align: left; margin-top: 10px; margin-bottom: 20px;'>"
                    f"<a href='data:chemical/x-pdb;base64,{b64}' download='{trimmed_name}.pdb' "
                    f"style='background-color: #d9c0a3; color: black; text-decoration: none; padding: 10px 20px; border-radius: 5px; font-size: 14px;'>"
                    f"Download Docked File (.pdb)</a>"
                    f"</div>",
                    unsafe_allow_html=True
                )
